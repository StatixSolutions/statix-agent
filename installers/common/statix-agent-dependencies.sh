...
ubuntu_packages() {
  printf '%s\n' \
    ca-certificates curl cloud-image-utils dnsmasq gawk iproute2 iptables libnginx-mod-stream lxc lxc-templates nginx procps util-linux \
...
required_commands() {
  printf '%s\n' \
    awk curl flock sha256sum systemctl ip ss sysctl tar sudo dnsmasq iptables nginx \
