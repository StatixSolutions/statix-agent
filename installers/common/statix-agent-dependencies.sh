...
ubuntu_packages() {
  printf '%s\n' \
    ca-certificates curl cloud-image-utils dnsmasq iproute2 iptables lxc lxc-templates \
...
arch_packages() {
  printf '%s\n' \
    ca-certificates curl dnsmasq iproute2 iptables lxc xz pciutils sudo \
...
required_commands() {
  printf '%s\n' \
    curl sha256sum systemctl ip ss tar sudo dnsmasq iptables \
