...
ubuntu_packages() {
  printf '%s\n' \
    ca-certificates curl cloud-image-utils dnsmasq gawk iproute2 iptables lxc lxc-templates procps util-linux \
...
arch_packages() {
  printf '%s\n' \
    ca-certificates curl dnsmasq gawk iproute2 iptables lxc procps-ng util-linux xz pciutils sudo \
...
required_commands() {
  printf '%s\n' \
    awk curl flock sha256sum systemctl ip ss sysctl tar sudo dnsmasq iptables \
